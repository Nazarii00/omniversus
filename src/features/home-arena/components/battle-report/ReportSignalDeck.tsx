...
type RadarMetrics = ReturnType<typeof buildRadarMetrics>;
...
function RadarPlot({
  ...
  return (
    ...
    >
      ...
      />
      {metrics.map((metric, index) => {
        const label = radarCoordinate(index, metrics.length, RADAR_LABEL_RADIUS);
...
function radarPolygon(
  metrics: ReturnType<typeof buildRadarMetrics>,
  side: "A" | "B",
) {
  return metrics
    .map((metric, index) => {
