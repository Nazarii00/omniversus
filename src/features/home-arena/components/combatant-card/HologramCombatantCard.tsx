...
import {
  BattleRevealCanvases,
  useBattleRevealAnimation,
  type BattleRevealConfig,
} from "./BattleRevealOverlay";
...
export default function HologramCombatantCard({
  ...
  const [flipState, setFlipState] = useState<{
    ...
    isFlipped: boolean;
  }>({ runId: undefined, isFlipped: false });
  const reveal = useBattleRevealAnimation(card, battleReveal);
  const revealRunId = battleReveal?.runId;
  const isFlipped =
    flipState.runId === revealRunId ? flipState.isFlipped : false;
  ...
  function activateCard() {
    if (
      reveal.isDestroyed ||
      (reveal.phase !== "idle" && reveal.phase !== "verdict")
    ) {
      return;
    }
    ...
    setFlipState((current) => ({
      runId: revealRunId,
      isFlipped: current.runId === revealRunId ? !current.isFlipped : true,
    }));
  }
  ...
  return (
    <button
      ...
      className={`home-battle-reveal-card group relative block aspect-[5/7] w-full min-w-0 cursor-pointer text-left outline-none [perspective:1500px] ${className}`}
      data-reveal-destroyed={reveal.isDestroyed ? "true" : "false"}
      data-reveal-phase={reveal.phase}
      data-reveal-role={reveal.role ?? "none"}
      style={themeStyle}
    >
      <div className="home-battle-reveal-card__body absolute inset-0">
        <div
      ...
      <BattleRevealCanvases
        damageCanvasRef={reveal.damageCanvasRef}
        flashCanvasRef={reveal.flashCanvasRef}
      />
      {reveal.phase === "verdict" && reveal.role ? (
        <>
          <span className="home-battle-reveal-clean-flash" aria-hidden="true" />
          <span className="home-battle-reveal-stamp" aria-hidden="true">
            {reveal.role === "winner" ? "WINNER" : "DEFEATED"}
          </span>
        </>
      ) : null}
    </button>
