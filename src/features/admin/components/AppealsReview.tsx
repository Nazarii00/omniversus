  ...
  return {
    reportLabel:
      reportTitle || reportId
        ? [reportTitle, reportId].filter(Boolean).join(" / ")
        : null,
    targetLabel:
      targetType || targetId
        ? [
...
function AppealContext({ appeal }: { appeal: Appeal }) {
  const context = appealContext(appeal);
  ...
  return (
    <div className={styles.appealContext}>
      {context.reportLabel ? (
        <p>
          <span>Report</span>
          {context.reportLabel}
      ...
      {context.targetText ? (
        <blockquote>{context.targetText}</blockquote>
      ) : null}
    </div>
  );
}
    ...
    <article className={styles.appealCard}>
      ...
      <dl className={styles.appealMeta}>
        ...
        <div>
          <dt>Submitter</dt>
          <dd>
            {appeal.submitterName ?? appeal.submitterContact ?? "Unknown"}
          </dd>
        </div>
      </dl>
  ...
  return (
    <section className={styles.section}>
      ...
      <div className={styles.appealStack}>
        {appeals.length ? (
          appeals.map((appeal) => (
            <AppealCard appeal={appeal} key={appeal.id} />
          ))
        ) : (
